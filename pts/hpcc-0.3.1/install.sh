...
# HPL.dat generation
N=\$(echo \"scale=0;sqrt(\${OMP_NUM_THREADS}*\${SYS_MEMORY}*128)\" |bc -l)
PQ=0
P=\$(echo \"scale=0;sqrt(\$OMP_NUM_THREADS)\" |bc -l)
Q=\$P
PQ=\$((\$P*\$Q))

while [ \$PQ -ne \$OMP_NUM_THREADS ]; do
    Q=\$((\$OMP_NUM_THREADS/\$P))
    PQ=\$((\$P*\$Q))
    if [ \$PQ -ne \$OMP_NUM_THREADS ] && [ \$P -gt 1 ]; then P=\$((\$P-1)); fi
done

NB=\$((256 - 256 % \$OMP_NUM_THREADS))
