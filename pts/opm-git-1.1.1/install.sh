...
CMAKE_EXTRAS="-DERT_ROOT=$HOME/ert -DPTSCOTCH_ROOT=$HOME/opt -DZOLTAN_ROOT=$HOME/opt -DPARMETIS_ROOT=$HOME/opt"
...
then
	mpirun --allow-run-as-root -np \$OMP_NUM_THREADS \$HOSTFILE ./opm-upscaling/build/bin/upscale_relperm_benchmark > \$LOG_FILE 2>&1
elif [ \$@ = \"flow_mpi_norne\" ]
then
	cd opm-data/norne
	../../opm-autodiff/build/bin/flow_mpi NORNE_ATW2013.DATA > \$LOG_FILE 2>&1
fi
