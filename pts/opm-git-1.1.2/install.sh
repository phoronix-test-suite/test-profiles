...
then
	mpirun \$MPIRUN_AS_ROOT_ARG -np \$OMP_NUM_THREADS \$HOSTFILE ./opm-upscaling/build/bin/upscale_relperm_benchmark > \$LOG_FILE 2>&1
elif [ \$@ = \"flow_mpi_norne\" ]
then
	cd opm-data/norne
	mpirun \$MPIRUN_AS_ROOT_ARG -np \$OMP_NUM_THREADS \$HOSTFILE ../../opm-autodiff/build/bin/flow_mpi NORNE_ATW2013.DATA > \$LOG_FILE 2>&1
fi
