...
git clone https://github.com/OPM/ewoms.git
...
then
	cd omega-opm
	nice mpirun \$MPIRUN_AS_ROOT_ARG -np \$OMP_NUM_THREADS --map-by socket --report-bindings \$HOSTFILE ../opm-simulators/build/bin/flow_ebos OMEGA-0.DATA > \$LOG_FILE 2>&1
