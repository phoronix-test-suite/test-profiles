...
then
	cd opm-data/norne
	nice mpirun \$MPIRUN_AS_ROOT_ARG -np \$OMP_NUM_THREADS --map-by socket --report-bindings \$HOSTFILE ../../opm-autodiff/build/bin/flow_mpi NORNE_ATW2013.DATA > \$LOG_FILE 2>&1
fi
